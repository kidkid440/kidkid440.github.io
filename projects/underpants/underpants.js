...
*/
_.each = function(collection, bunch){
    if(Array.isArray(collection)){
       for(var i = 0; i < collection.length; i++){
           bunch(collection[i], i, collection);
       } 
    }else{
        for(var key in collection){
        bunch(collection[key], key, collection);
        }
    }
};
...
_.reject = function(array, stop){
    let box = [];
  for(let i = 0; i < array.length; i++){
     let values = stop(array[i], i, array);
       if(values === false){
          box.push(array[i]);
       }
  }
...
_.partition = function(array, bunch){
    let  result = [];
    let box = [];
    let bigBox = [];
    for(let i = 0; i < array.length; i++){
